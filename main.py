...
from models import db, Classe, Analyse, User, Prompt # <-- ON IMPORTE PROMPT

main = Blueprint('main', __name__)

# --- ROUTES (login, logout, accueil, analyser, configuration, historique_classe inchangées ou adaptées) ---
...
def accueil():
    ...
    try:
        ...
        flash(f"La base de données n'est pas initialisée. Veuillez visiter /init-db-manuellement. Erreur: {e}", "warning")
        return render_template('accueil.html', classes=[])
...
def analyser():
    ...
    if request.method == 'POST':
        ...
        try:
            # --- RÉCUPÉRATION DU PROMPT ACTIF DE LA BDD ---
            active_prompt = Prompt.query.filter_by(is_active=True).first()
            if not active_prompt:
                raise ValueError("Aucun prompt actif n'est défini. Veuillez en définir un dans la configuration des prompts.")
            ...
            contexte_trimestre = ""
            if trimestre == 1: contexte_trimestre = "C'est le début de l'année, l'appréciation doit être encourageante et fixer des objectifs clairs pour les deux trimestres restants."
            elif trimestre == 2: contexte_trimestre = "C'est le milieu de l'année. L'appréciation doit faire le bilan des progrès par rapport au T1 et motiver pour le dernier trimestre."
            elif trimestre == 3: contexte_trimestre = "C'est la fin de l'année. L'appréciation doit être un bilan final, tenir compte de l'évolution sur l'année et donner des conseils pour la poursuite d'études."

            liste_appreciations = "\n".join([f"- {item['matiere']} ({item['moyenne']}): {item['commentaire']}" for item in donnees_structurees['appreciations_matieres']])
            
            # --- UTILISATION DU PROMPT RÉCUPÉRÉ ---
            prompt_systeme = active_prompt.system_message
            prompt_utilisateur = active_prompt.user_message_template.format(
                nom_eleve=nom_eleve,
                trimestre=trimestre,
                contexte_trimestre=contexte_trimestre,
                appreciations_precedentes=appreciations_precedentes,
                liste_appreciations=liste_appreciations
            )
            ...
            # Sauvegarde/Mise à jour de l'analyse (inchangée)
    ...
    analyses_faites = {} # Pour marquer les élèves déjà analysés par trimestre
    for analyse in Classe.query.get(classe_id).analyses: # Utilise la relation
...
# --- NOUVELLES ROUTES POUR LA GESTION DES PROMPTS ---
@main.route('/prompts')
@login_required
def list_prompts():
...
def add_prompt():
    if request.method == 'POST':
        ...
        user_message_template = request.form.get('user_message_template')
...
def delete_prompt(prompt_id):
    prompt = Prompt.query.get_or_404(prompt_id)
    if prompt.is_active:
        flash("Impossible de supprimer un prompt actif. Veuillez en activer un autre d'abord.", "danger")
    else:
...
# --- ROUTES (configuration, supprimer_classe, historique_classe inchangées) ---
...
# --- ROUTE SECRÈTE POUR INITIALISER LA BDD (maintenant dans main.py) ---
...
def init_db_manually():
    try:
        db.drop_all()
        db.create_all()
        # Créer un prompt par défaut lors de l'initialisation
        default_prompt = Prompt.query.filter_by(name="Default Prompt").first()
...
Sous le séparateur, justifie chaque idée clé avec des citations brutes des commentaires DU TRIMESTRE ACTUEL."""
            ...
            db.session.commit()
            set_active_prompt(default_prompt.id) # Activer le prompt par défaut

...
# --- ROUTES (login, logout, accueil inchangées) ---
...
def analyser():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # --- ADAPTATION DU PROMPT EN FONCTION DU TRIMESTRE ---
            contexte_trimestre = ""
            if trimestre == 1:
                contexte_trimestre = "C'est le début de l'année, l'appréciation doit être encourageante et fixer des objectifs clairs pour les deux trimestres restants."
            elif trimestre == 2:
                contexte_trimestre = "C'est le milieu de l'année. L'appréciation doit faire le bilan des progrès par rapport au T1 et motiver pour le dernier trimestre."
            elif trimestre == 3:
                contexte_trimestre = "C'est la fin de l'année. L'appréciation doit être un bilan final, tenir compte de l'évolution sur l'année et donner des conseils pour la poursuite d'études."
            ...
            **Partie 2 : Justifications**
            ...
            # On vérifie s'il existe déjà une analyse pour cet élève et ce trimestre, et on la met à jour
    ...
    for analyse in classe.analyses:
        if analyse.nom_eleve not in analyses_faites:
            analyses_faites[analyse.nom_eleve] = set()
...
def historique_classe(classe_id):
    ...
    for nom_eleve in eleves_tries:
        analyses_par_eleve[nom_eleve] = sorted(
            [a for a in classe.analyses if a.nom_eleve == nom_eleve],
